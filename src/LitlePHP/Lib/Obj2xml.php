...
class Obj2xml
{
  ...
  public static function toXml($data, $hash_config, $type, $rootNodeName = 'litleOnlineRequest')
  {
    ...
    $xml->addAttribute('version', LitleConstants::$CURRENT_XML_VERSION);
  ...
  {
    if(!self::$config instanceof LitleConfig)
    {
      throw new \Exception("Config is not set");
    }
    self::$config->mergeIn($data);
    if(!self::$config->validate())
    {
      throw new \InvalidArgumentException("One or more fields is not set on the config");
    }
    if($type == 'updateSubscription' || $type == 'cancelSubscription' || $type == 'createPlan' || $type == 'updatePlan')
    {
      self::$config->reportGroup = null;
    }
    return self::$config->getAsArray();
  }
}
